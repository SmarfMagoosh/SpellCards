...
if(meta.spellBlocks.size == 0) {
    ...
    for (let i = 0; i < meta.spells.length; i++) {
        if (meta.spellBlocks.has(i)) {
            ...
            row.innerHTML = "<td>" + meta.spells[i].name + "</td><td><button onclick='removeSpell(this)' class='list-button' id='index" + i + "'>Remove</button></td>"
        }
    }
}

function removeSpell(node) {
    meta.spellBlocks.delete(parseInt(node.id.substring(5)));
